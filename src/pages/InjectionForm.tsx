...
=======
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (pecasNG === 0 && totalPecas > 0 && photos.length === 0) { toast.error(t("injectionForm.okRequirePhoto")); return; }
    if (pecasNG === 0 && totalPecas > 0 && !photoType) { toast.error(t("injectionForm.selectPhotoTypeError")); return; }
    setLoading(true);
    try {
      const formData = new FormData(e.currentTarget);
      const payload: Record<string, any> = {
...
=======
          <div className="form-section">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2"><Label htmlFor="nome">{t("common.name")} *</Label><Input id="nome" name="nome" required value={profile?.full_name || ""} readOnly className="bg-muted" /></div>
              <div className="space-y-2"><Label htmlFor="data">{t("common.date")} *</Label><Input id="data" name="data" type="date" required defaultValue={defaults.data || ""} key={defaults.data || "new"} /></div>
          ...
          <div className="form-section">
            <h3 className="form-section-title">{t("injectionForm.partData")}</h3>
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                <Label>{t("injectionForm.tryoutReason")} *</Label>
                <Select value={razaoTryout} onValueChange={(v) => { setRazaoTryout(v); if (v !== "Outro") setRazaoTryoutOutro(""); }}>
                  <SelectTrigger><SelectValue placeholder={t("injectionForm.selectReason")} /></SelectTrigger>
                  <SelectContent>{["EO", "4M", "Melhoria", "Correção", "Novo Carro", "Outro"].map((opt) => <SelectItem key={opt} value={opt}>{opt}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
                <Label>{razaoTryout === "Outro" ? `${t("injectionForm.specify")} *` : t("injectionForm.detail")}</Label>
                <Input placeholder={razaoTryout === "Outro" ? t("injectionForm.specifyPlaceholder") : t("injectionForm.detailPlaceholder")} value={razaoTryoutOutro} onChange={(e) => setRazaoTryoutOutro(e.target.value)} required={razaoTryout === "Outro"} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="qtdTryout">{t("injectionForm.tryoutCount")}*</Label>
                <Select name="qtdTryout" required defaultValue={defaults.qtd_tryout?.toString() || ""} key={`qt-${defaults.qtd_tryout}`}>
                  <SelectTrigger><SelectValue placeholder={t("common.select")} /></SelectTrigger>
                  <SelectContent>{[1,2,3,4,5,6].map(n => <SelectItem key={n} value={String(n)}>{n}ª</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div className="space-y-2"><Label htmlFor="totalPecas">{t("injectionForm.totalParts")} *</Label><Input id="totalPecas" type="number" required min={0} placeholder="0" value={totalPecas || ""} onChange={(e) => setTotalPecas(Number(e.target.value) || 0)} /></div>
              <div className="space-y-2"><Label htmlFor="pecasNG">{t("injectionForm.partsNG")} *</Label><Input id="pecasNG" type="number" required min={0} max={totalPecas} placeholder="0" value={pecasNG || ""} onChange={(e) => setPecasNG(Math.min(Number(e.target.value) || 0, totalPecas))} /></div>
              <div className="space-y-2"><Label htmlFor="pecasOK">{t("injectionForm.partsOK")}</Label><Input id="pecasOK" type="number" value={pecasOK} readOnly className="bg-muted font-semibold text-accent" /></div>
              <div className="space-y-2 sm:col-span-2">
                <Label htmlFor="rate">{t("injectionForm.rate")}</Label>
                <div className="flex items-center gap-3">
                  <Input id="rate" type="text" value={`${rate}%`} readOnly className="bg-muted font-semibold text-accent max-w-[160px]" />
                  <div className="h-3 flex-1 rounded-full bg-muted overflow-hidden"><div className="h-full rounded-full bg-accent transition-all duration-300" style={{ width: `${Math.min(parseFloat(rate), 100)}%` }} /></div>
>>>>>>> 853a538787cf446c7d01e628ea96edf722a8086f
                </div>
              </div>
            </div>
...
=======
          <div className="form-section">
            <h3 className="form-section-title">{t("injectionForm.processParams")}</h3>
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2"><Label htmlFor="materiaPrima">{t("injectionForm.rawMaterial")} *</Label><Input id="materiaPrima" name="materiaPrima" required placeholder={t("injectionForm.materialType")} defaultValue={defaults.materia_prima || ""} key={`mp-${defaults.materia_prima}`} /></div>
              <div className="space-y-2"><Label htmlFor="injetora">{t("injectionForm.injector")} *</Label><Input id="injetora" name="injetora" required placeholder={t("injectionForm.injectorId")} defaultValue={defaults.injetora || ""} key={`inj-${defaults.injetora}`} /></div>
              <div className="space-y-2"><Label htmlFor="tonelagem">{t("injectionForm.tonnage")} *</Label><Input id="tonelagem" name="tonelagem" type="number" required placeholder={t("injectionForm.inTons")} defaultValue={defaults.tonelagem || ""} key={`ton-${defaults.tonelagem}`} /></div>
              <div className="space-y-2"><Label htmlFor="cycleTime">{t("injectionForm.cycleTime")} *</Label><Input id="cycleTime" name="cycleTime" type="number" step="0.1" required placeholder={t("injectionForm.seconds")} defaultValue={defaults.cycle_time || ""} key={`ct-${defaults.cycle_time}`} /></div>
              <div className="space-y-2"><Label htmlFor="coolingTime">{t("injectionForm.coolingTime")} *</Label><Input id="coolingTime" name="coolingTime" type="number" step="0.1" required placeholder={t("injectionForm.seconds")} defaultValue={defaults.cooling_time || ""} key={`cool-${defaults.cooling_time}`} /></div>
              <div className="space-y-2"><Label htmlFor="weight">{t("injectionForm.weight")} *</Label><Input id="weight" name="weight" type="number" step="0.01" required placeholder={t("injectionForm.grams")} defaultValue={defaults.weight || ""} key={`w-${defaults.weight}`} /></div>
            </div>
          </div>

          <div className="form-section">
            <h3 className="form-section-title">{t("injectionForm.evaluation")}</h3>
            <div className="space-y-4">
              <div className="space-y-2"><Label htmlFor="dimensional">{t("injectionForm.dimensional")} *</Label><Input id="dimensional" name="dimensional" required placeholder={t("injectionForm.dimensionalResult")} defaultValue={defaults.dimensional || ""} key={`dim-${defaults.dimensional}`} /></div>
              <div className="space-y-2"><Label htmlFor="comentarios">{t("injectionForm.generalComments")}</Label><Textarea id="comentarios" name="comentarios" placeholder={t("common.additionalObs")} rows={4} defaultValue={defaults.comentarios || ""} key={`com-${defaults.comentarios}`} /></div>
            </div>
          </div>

          <div className="form-section">
            <div className="flex items-center justify-between mb-3">
              <h3 className="form-section-title mb-0">{t("injectionForm.defects")}</h3>
              <Button type="button" variant="outline" size="sm" onClick={addDefect} className="gap-1.5" disabled={pecasNG === 0}><Plus className="w-4 h-4" /> {t("injectionForm.addDefect")}</Button>
            </div>
            {pecasNG === 0 && <div className="text-sm text-muted-foreground text-center py-6 border border-border rounded-lg bg-muted/30">{t("injectionForm.defectsBlocked")}</div>}
            {pecasNG > 0 && defects.length === 0 && <div className="text-sm text-destructive text-center py-6 border border-destructive/30 rounded-lg bg-destructive/5">⚠️ {t("injectionForm.defectsRequired")}</div>}
            {pecasNG > 0 && defects.length === 0 && <p className="text-sm text-muted-foreground text-center py-6 border border-dashed border-border rounded-lg">{t("injectionForm.noDefects")}</p>}
>>>>>>> 853a538787cf446c7d01e628ea96edf722a8086f
...
=======
                    <span className="text-sm font-semibold text-foreground">{t("injectionForm.defectNumber")} #{idx + 1}</span>
                  ...
                  <div className="space-y-2"><Label>{t("injectionForm.defectDescription")} *</Label><Input required placeholder={t("injectionForm.describeDefect")} value={defect.description} onChange={(e) => updateDefect(idx, "description", e.target.value)} /></div>
                  <div className="space-y-2">
                    <Label>{t("injectionForm.needsImprovement")}</Label>
                    <Select value={defect.needs_improvement ? "sim" : "nao"} onValueChange={(v) => updateDefect(idx, "needs_improvement", v === "sim")}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="sim">{t("injectionForm.yes")}</SelectItem><SelectItem value="nao">{t("injectionForm.no")}</SelectItem></SelectContent></Select>
                  </div>
                  {defect.needs_improvement && (
                    <div className="space-y-2 opacity-0 animate-fade-in">
                      <Label>{t("injectionForm.improvementCategory")} *</Label>
                      <Select required value={defect.improvement_category} onValueChange={(v) => updateDefect(idx, "improvement_category", v)}>
                        <SelectTrigger><SelectValue placeholder={t("injectionForm.selectCategory")} /></SelectTrigger>
                        <SelectContent>
...
=======
                  <div className="space-y-2">
                    <Label>{t("injectionForm.failureMode")}</Label>
                    <Select value={defect.failure_mode} onValueChange={(v) => updateDefect(idx, "failure_mode", v)}>
                      <SelectTrigger><SelectValue placeholder={t("injectionForm.selectFailureMode")} /></SelectTrigger>
                      <SelectContent>
                  ...
                  <div className="space-y-2">
                    <Label className="flex items-center gap-1.5"><Camera className="w-4 h-4" /> {t("injectionForm.defectPhotos")}</Label>
                    ...
                    <Button type="button" variant="outline" size="sm" onClick={() => defectFileRefs.current[idx]?.click()} className="w-full border-dashed border-2 h-12 text-muted-foreground hover:text-foreground hover:border-accent"><Camera className="w-4 h-4 mr-2" /> {t("common.addPhotos")}</Button>
                    {defect.photos.length > 0 && (
                      <div className="grid grid-cols-4 gap-2 mt-2">
                        {defect.photos.map((photo, pi) => (
...
=======
          <div className={`form-section ${pecasNG > 0 ? "opacity-50 pointer-events-none" : ""}`}>
            <h3 className="form-section-title"><Camera className="w-5 h-5" /> {t("injectionForm.pieceOK")} {pecasNG > 0 && <span className="text-xs text-muted-foreground ml-2">({t("injectionForm.enabledOnlyNoNG")})</span>}</h3>
            {pecasNG === 0 && totalPecas > 0 && (
              <div className="mb-4 space-y-2">
                <Label>{t("injectionForm.photoType")} *</Label>
                <Select value={photoType} onValueChange={setPhotoType} disabled={pecasNG > 0}><SelectTrigger><SelectValue placeholder={t("injectionForm.selectPhotoType")} /></SelectTrigger><SelectContent><SelectItem value="peca_referencia">{t("injectionForm.referencePart")}</SelectItem><SelectItem value="peca_final">{t("injectionForm.finalPart")}</SelectItem></SelectContent></Select>
              </div>
            )}
            <input ref={fileInputRef} type="file" accept="image/*" multiple className="hidden" onChange={handlePhotoUpload} disabled={pecasNG > 0} />
            <Button type="button" variant="outline" onClick={() => fileInputRef.current?.click()} disabled={pecasNG > 0} className="w-full border-dashed border-2 h-20 text-muted-foreground hover:text-foreground hover:border-accent disabled:opacity-50 disabled:cursor-not-allowed">
